---
title: "Spatio-temporal Modelling, Lab 2, 2022-10-26"
format: html
editor: visual
---
...
Three parameters were fitted. The following periodic model is used: $$x_1 + x_2 \sin(\pi(t + x_3)/12)$$
...
No, there is no significant partial autocorrelation for lags above 0. Please note that the value range (y-axis) in this acf plot is much smaller than in the previous plot!
:::
...
```{r}
#Exercise 8.1 --> fit the periodic component
...
Trend equation: $$74.635633 + 19.874064 * sin(pi*(t+1.647808)/12)$$

### *Exercise 8.2:* {.unnumbered}

```{r}
#Exercise 8.2 -> plot the humidity data and the fitted model
...
```{r}
#Exercise 8.3 -> detrend and check residuals
##calculate residuals
...
```{r}
#Exercise 8.4 -> compute and plot acf for the residuals
humidity.an.ar9 = arima(humidity.an, c(9,0,0))
acf(residuals(humidity.an.ar9), type = "partial")
```

There is no significant partial autocorrelation for lags above 0.
:::
...
#### *Eercise 10* {.unnumbered}

::: callout-caution
## Exercise 10

*Where does now, for long-term forecasts, the predicted value convergeto? Explain the difference to the upper model.*
:::
